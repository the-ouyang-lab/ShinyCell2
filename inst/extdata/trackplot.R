...
read_coldata = function(bws = NULL, sample_names = NULL, build = "hg38", input_type = "bw"){
  ...
  if(is.null(sample_names)){
    bw_sample_names = unlist(sub("\\.[^.]*$", "", basename(bws)))
...
track_plot = function(summary_list = NULL,
      ...
      for(idx in 1:length(summary_list)){
        ...
        if(nrow(x) == 0){
          if(track_names_to_left){
            ...
            title(main = names(summary_list)[idx], adj = track_names_pos, font.main = 3, cex.main = gene_fsize * 0.75)
          }
          next
        }
  ...
  text(x = lab_at, y = 0.2, labels = lab_at_lab, xpd = FALSE, cex = gene_fsize)
  #axis(side = 1, at = lab_at, lty = 2, line = -3)
  text(x = end, y = 0.85, labels = paste0(chr, ":", start, "-", end), adj = 1, xpd = TRUE, cex = gene_fsize)
...
profile_extract = function(colData = NULL, bed = NULL, ucsc_assembly = TRUE, startFrom = "start", binSize = 50,
  ...
  #op_dir = tempdir() #For now, commented out by cbj
  # op_dir = "./tmp" # addedby cbj
  # if(!dir.exists(paths = op_dir)){
  #   dir.create(path = op_dir, showWarnings = FALSE, recursive = TRUE)
  # } # added by cbj
  # Create a more specific temporary directory
  op_dir = file.path(getwd(), "tmp_profile_extract")
  dir.create(path = op_dir, showWarnings = FALSE, recursive = TRUE)
